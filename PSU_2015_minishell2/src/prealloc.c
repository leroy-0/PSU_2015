/*
** prealloc.c for prealloc in
**
** Made by leroy_0
** Login   <leroy_0@epitech.net>
**
** Started on  Thu Apr 21 10:06:51 2016 leroy_0
** Last update Thu Apr 21 10:06:59 2016 leroy_0
*/
...
void		my_setenvir_antes(t_struct_d *data, char *arg1, char *arg2, int a)
{
  ...
  b = 0;
  c = 0;
  data->tmpenv[a] = my_x_malloc(sizeof(char) * (my_strlen(arg1)
					   + my_strlen(arg2) + 2));
  while (arg1 && arg1[b])
    {
...
char		**my_other_set(t_struct_d *data, char *arg1, char *arg2)
{
  ...
  while (data->tmpenv[a])
    a++;
  newenv = my_x_malloc(sizeof(char *) * (a + 2));
  a = 0;
  while (data->tmpenv[a])
    {
      newenv[a] = my_x_malloc(sizeof(char) * my_strlen(data->tmpenv[a]) + 1);
      my_strcpy(newenv[a], data->tmpenv[a]);
      a++;
    }
  newenv[a] =
    my_x_malloc(sizeof(char) * (my_strlen(arg1) + my_strlen(arg2) + 2));
  if (arg1)
...
void		my_final_set(t_struct_d *data, char *arg1, char *arg2, int a)
{
  ...
  while (newenv[b])
    b++;
  data->tmpenv = my_x_malloc(sizeof(char *) * (b + 1));
  a = 0;
  while (newenv[a])
    {
      data->tmpenv[a] = my_x_malloc(sizeof(char) * my_strlen(newenv[a]) + 1);
