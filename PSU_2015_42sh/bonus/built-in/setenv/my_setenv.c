...
** Last update Sun Jun  5 23:43:59 2016 longle_h
*/
...
static char	*fill_look(char *str, int *cpt)
{
  ...
  if ((look = my_x_malloc(sizeof(char) * (strlen(str) + 1))) == NULL)
    return (NULL);
...
char		**my_putenvend(char *look, char *s, char **myenv)
{
  ...
  while (myenv && myenv[i])
    i++;
  if ((tmpenv = my_x_malloc(sizeof(char *) * (i + 2))) == NULL)
    return (NULL);
  i = 0;
  while (myenv && myenv[i])
    {
...
static char	*fill_tmp(char *str, int *cpt)
{
  ...
  cpt_tmp = 0;
  if ((tmp = my_x_malloc(sizeof(char) * (strlen(str) + 1))) == NULL)
    return (NULL);
  while (str && str[*cpt] && str[*cpt] != ' ')
    {
      tmp[cpt_tmp] = str[*cpt];
...
char		**my_setenv(char *str, char **myenv)
{
  ...
  if (str && str[cpt] == ' ')
    ...
    return (my_puterr("setenv: Variable name must\
